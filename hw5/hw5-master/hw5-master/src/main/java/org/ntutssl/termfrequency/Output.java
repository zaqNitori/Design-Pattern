...
public class Output implements EventListener{
  ...
  public void onEvent(EventType eventType, String event) {
    if(eventType==EventType.OUTPUT) outputEvent(event);  
  }

  private void outputEvent(String output){
    for(String s : output.split("\n")){
      String key = s.split(":")[0];
      Integer value = Integer.valueOf(s.split(": ")[1]);
      this.wordFrequencyMap.put(key, value);
    }
    if(this.wordFrequencyMap.isEmpty()) throw new WordFrequencyException("Word not found.");
    List<String> outputList = this.sort(this.wordFrequencyMap, this.order);
    if( range < 1 ) throw new WordFrequencyException("Out of range! The range should be from 1 to " + outputList.size() + ".");
    try{
      outputList=outputList.subList(0, range);
    }catch(IndexOutOfBoundsException e){
      throw new WordFrequencyException("Out of range! The range should be from 1 to " + outputList.size() + ".");
    }
  ...
  private List<String> sort(Map<String, Object> wordFrequencyMap, String order){
    List<Map.Entry> wordFrequencyEntryList = new ArrayList<>(this.wordFrequencyMap.entrySet());
        ...
        Comparator< Map.Entry> sortByValue = (e1,e2)->{ 
            if(order.equals("des")) return (Integer.parseInt(String.valueOf(e2.getValue())) - Integer.parseInt(String.valueOf(e1.getValue())));
            else if(order.equals("asc"))  return (Integer.parseInt(String.valueOf(e1.getValue())) - Integer.parseInt(String.valueOf(e2.getValue())));
            else throw new WordFrequencyException("The order should be \"asc\" or \"des\".");
        };
